...
fn returns_directive_hover_for_keyword_only() {
    ...
    let value_offset = TextSize::from(source.find("deno").expect("directive value should exist") as u32);
    ...
    assert!(info.docs.as_deref().is_some_and(|docs| docs.contains("Current value: `deno`")));
...
fn returns_guard_probe_hover() {
    ...
    let argument_offset = TextSize::from(source.find("macos").expect("probe argument should exist") as u32);

    let info = hover(&snapshot, offset).expect("hover should exist");

    assert_eq!(info.kind, LspHoverKind::GuardProbe);
    assert_eq!(info.signature, "@os(\"macos\")");
    assert!(info.docs.as_deref().is_some_and(|docs| docs.contains("Current argument: `macos`")));
...
fn returns_shell_operator_hover() {
    ...
    let snapshot = DocumentSnapshot::new("file:///workspace/Onlyfile", 1, source);
    let offset = TextSize::from(source.find("shell?=").expect("shell operator should exist") as u32);
    let value_offset = TextSize::from(source.find("bash").expect("shell value should exist") as u32);
...
fn returns_interpolation_hover() {
    ...
    let snapshot = DocumentSnapshot::new("file:///workspace/Onlyfile", 1, source);
    let offset = TextSize::from(source.find("{{name}}").expect("interpolation should exist") as u32);
